...
const LINES = [
    ...
    { content: "}", type: "plain" },
    { content: "\n\\author{", type: "cmd" },
    { content: "Alice, Bob, Carol", type: "accent" },
    { content: "}", type: "plain" },
    { content: "\n\n\\begin", type: "cmd" },
    { content: "{document}", type: "plain" },
    { content: "\n  \\maketitle", type: "cmd" },
    { content: "\n\n  The Gaussian integral $\\int_{-\\infty}^{\\infty} e^{-x^2}\\,dx = \\sqrt{\\pi}$.", type: "math" },
    { content: "\n\\end", type: "cmd" },
    { content: "{document}", type: "plain" },
];
...
const MockEditor = () => {
    ...
    const totalChars = LINES.reduce((s, l) => s + l.content.length, 0);

    useEffect(() => {
        if (visibleChars >= totalChars) return;
        const t = setTimeout(() => setVisibleChars((v) => v + 1), 28);
    ...
    for (const { content, type } of LINES) {
        const visible = Math.max(0, Math.min(content.length, visibleChars - rendered));
        const slice = content.slice(0, visible);
        rendered += content.length;
        if (!slice) break;
        spans.push(
            <span key={rendered} className={COLOR[type as keyof typeof COLOR]}>
                {slice}
            </span>,
        );
    ...
    return (
        <pre className="overflow-hidden p-6 font-mono text-sm leading-7 whitespace-pre-wrap select-none text-base-content/70">
            {spans}
            <span className="inline-block ml-0.5 h-4 align-middle animate-pulse w-[2px] bg-primary" />
        </pre>
    );
};
...
const Hero = () => {
    ...
    return (
        <div
            className="overflow-x-hidden min-h-screen bg-base-100"
        ...
        >
            ...
            <section className="grid relative z-10 grid-cols-1 gap-12 items-center py-24 px-6 mx-auto max-w-7xl min-h-screen lg:grid-cols-2">
                ...
                <div className="flex flex-col items-start">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                        <a href="/sign-up" className="gap-2 px-8 btn btn-primary btn-lg">
                            Get started
                            <LuArrowRight />
                        </a>
                        ...
                        <div className="flex gap-2 items-center py-3 px-4 border-b bg-base-300 border-base-300/60">
                            ...
                            <div className="flex gap-3 items-center ml-auto">
                                ...
                                <div className="flex -space-x-1">
                                    {["bg-blue-400", "bg-pink-400", "bg-amber-400"].map((c, i) => (
                                        <div
                                            key={i}
                                ...
                                <div className="gap-1 font-mono badge badge-success badge-xs">
                                    <span className="w-1 h-1 rounded-full animate-pulse bg-success-content" />3 online
                                </div>
                            </div>
                        </div>
            ...
            <section className="relative z-10 py-24 px-6 bg-base-200/50 border-y border-base-300">
                <div className="mx-auto max-w-5xl">
                    ...
                    <h2
                        className="mb-12 text-3xl font-bold text-center text-base-content"
                        style={{ fontFamily: "Georgia, serif" }}
                    >
                        ...
                        {[
                            ...
                            >
                                <div className="gap-4 card-body">
                                    ...
                                    <h3 className="text-sm card-title" style={{ fontFamily: "Georgia, serif" }}>
                                        {title}
                                    </h3>
            ...
            <section className="relative z-10 py-16 px-6 bg-base-200/40 border-y border-base-300">
                <div className="mx-auto max-w-3xl text-center">
                    <div
                        className="text-base italic leading-relaxed text-base-content/50"
                        style={{ fontFamily: "Georgia, serif" }}
                    >
